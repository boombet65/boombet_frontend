...
export const useBetStore = defineStore('betStore', () => {
  ...
  // Total odds only - backend handles the rest
   ...
   * Backend handles: total odds, potential win, tax, net payout
   * Frontend only sends: stake and selections
   */
  const placeBetWithBackend = async () => {
    ...
    try {
      // Prepare selections for backend - only send match_id, market_key, outcome_key
  ...
  return {
    ...
    // Getters - only what frontend needs
    slipCount,
    totalOdds,
    isSlipValid,
