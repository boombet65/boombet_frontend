...
<template>
  <div class="max-w-2xl mx-auto px-4 py-6">
    ...
    <div v-else-if="bet" class="space-y-4">
      ...
      <!-- Header card -->
      <div class="bg-cyan-900/20 border border-cyan-800/40 rounded-2xl p-5 relative">
        <div class="flex items-start justify-between mb-5">
         
            <p class="text-xs text-cyan-600 font-mono mb-1">Bet ID</p>
            <p class="text-base font-black text-yellow-400 font-mono">#{{ bet.bet_ticket }}</p>
     
          <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold border elevation-1 absolute -top-3 right-2"
        ...
        <div class="flex flex-col pt-4 border-t border-cyan-800/40">
          <div class="flex items-center justify-between mb-2">
            <p class="text-xs text-cyan-400 font-bold">Stake</p>
            <p class="text-sm font-bold text-cyan-100 font-mono">TZS {{ formatMoney(bet.stake) }}</p>
          </div>

           <div class="flex items-center justify-between mb-2">
            <p class="text-xs text-cyan-400 font-bold">Total Odds</p>
            <p class="text-sm font-bold text-cyan-100 font-mono">TZS {{ formatMoney(bet.totalOdds) }}</p>
          </div>
           <div class="flex items-center justify-between mb-2">
            <p class="text-xs text-cyan-400 font-bold">Potential Win</p>
            <p class="text-sm font-bold text-cyan-100 font-mono">TZS {{ formatMoney(bet.possibleWin) }}</p>
          </div>
           <div class="flex items-center justify-between mb-2">
            <p class="text-xs text-cyan-400 font-bold">Tax 12%</p>
            <p class="text-sm font-bold text-cyan-100 font-mono">TZS {{ formatMoney(bet.tax) }}</p>
          </div>
           <div class="flex items-center justify-between mb-2">
            <p class="text-xs text-cyan-400 font-bold">{{ bet.result === 'won' ? 'Net Won' : 'Net Return' }}</p>
            <p class="text-sm font-black font-bold" :class="bet.result === 'won' ? 'text-emerald-400' : 'text-cyan-600'">TZS TZS {{ formatMoney(bet.payout || bet.netPayout) }}</p>
          </div>
          ...
          <div v-for="(sel, i) in bet.selections" :key="i"
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center justify-between mt-1.5 text-xs text-cyan-700">
                ...
                <!-- ============ FORMAT SCORE ============ -->
                <span class="font-mono">{{ formatScore(sel.score) }}</span>
                <!-- ====================================== -->
              </div>
            </div>
          </div>
...
function formatScore(score) {
  ...
  // If score is already a string like "0-0" or "1-2"
  if (typeof score === 'string') {
    return score
  }
  
  // If score is an object like { home: 0, away: 0 }
  if (typeof score === 'object' && score !== null) {
    const home = score.home !== undefined ? score.home : score.homeScore
    const away = score.away !== undefined ? score.away : score.awayScore
  ...
  // If score is a number or something else
  return String(score)
}
// =============================================
    ...
    bet_ticket: bet.ticket_code,
      bet.value = {
        ...
        selections: storedBet.selections?.map(sel => {
          // ============ FORMAT SCORE FOR EACH SELECTION ============
          let score = sel.score || sel.match?.current_score || sel.match?.score || '—'
          
          // If score is an object, format it
          if (typeof score === 'object' && score !== null) {
            const home = score.home !== undefined ? score.home : score.homeScore
            const away = score.away !== undefined ? score.away : score.awayScore
            if (home !== undefined && away !== undefined) {
          ...
          return {
            ...
            status: sel.status || sel.match?.status,
        ...
        bet.value = {
          ...
          selections: data.selections?.map(sel => {
            ...
            return {
              ...
              status: sel.status || sel.match?.status,
              league: sel.league || sel.match?.league || '—',
              score: score // Now formatted as string
            }
          })
        }
